...
public class QueryMoreSoapRequest implements Request{
	...
	static final String defaultBatchSize="200";
	
	private String soapRequest=null;
	
	public QueryMoreSoapRequest(HashMap<String, String> requestFields) {
	...
	private String createSoapRequest(HashMap<String, String> requestFields){
		...
		try {
			...
			serializer.startTag(URN, QUERY_MORE);
			serializer.startTag(URN, QUERY_LOCATOR);
			serializer.text(requestFields.get(QUERY_LOCATOR));
			serializer.endTag(URN, QUERY_LOCATOR);
			serializer.endTag(URN, QUERY_MORE);
